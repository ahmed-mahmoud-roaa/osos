...
export default function Ui({ data }) {
  ...
  useEffect(() => {
    ...
    setShow('w-[35rem] max-h-screen')
    return () => setShow('w-[10rem] max-h-10')
  }, [])
  ...
  return (
    <div
      className={`searchContainer w-[20rem] bg-themeWhite-white top-2 relative rounded-xl text-sm shadow-[0_0_5px_1px_var(--themeGray-200)] overflow-hidden duration-1000 max-w-[90%]  ${show}`}
    >
              ...
              {section.title && (
                <span className="head flex items-center justify-between px-2.5 mb-2">
                  <span className="flex items-center">
                    ...
                    <span className="text-themeGray-700">{section.title}</span>
                  </span>
                  {section.viewAll && (
                    <button
